...
impl Step for BsanRT {
    ...
    fn run(self, builder: &Builder<'_>) -> Self::Output {
        ...
        let core_runtime_path = builder.ensure(BsanRTCore { compiler, target });
        let core_runtime_parent_dir = core_runtime_path.parent().unwrap();
        ...
        ldflags.push_all(format!("-L{}", &core_runtime_parent_dir.display()));
        ...
        if !is_dylib(&runtime.name) {
            let llvm_ar_bin = bindir.join(exe("llvm-ar", compiler.host));
            if let Err(_) = append_archive(builder, &llvm_ar_bin, &core_runtime_path, &runtime.path)
            {
                return None;
            }
        }
...
fn append_archive(
    ...
    let _ = Command::new(&llvm_ar).current_dir(&temp_dir).arg("-x").arg(source).status()?;
