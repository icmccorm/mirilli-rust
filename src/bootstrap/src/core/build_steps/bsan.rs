...
impl Step for BsanRT {
    ...
    fn run(self, builder: &Builder<'_>) -> Self::Output {
        ...
        let core_runtime = builder.ensure(BsanRTCore { compiler, target });
        let core_runtime = core_runtime.display();
        ...
        cfg.cxxflag(format!("-I{}", core_runtime));
        cfg.cflag(format!("-I{}", core_runtime));
        ...
        ldflags.push_all(format!("-L{core_runtime}"));
        ldflags.push_all(format!("-I{core_runtime}"));
        configure_cmake(
            builder,
            self.target,
            &mut cfg,
